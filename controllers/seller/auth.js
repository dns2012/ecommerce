...
router.post("/register", (req, res) => {
    let residenceId = req.body.residenceId;
    ...
    sellerModel.getByEmail(email, (results) => {
        if(results.length > 0) {
            res.status(200).json({
                status  : false,
            ...
            sellerModel.getByResidenceId(residenceId, (results) => {
                if(results.length > 0) {
                    res.status(200).json({
                        status  : false,
                        message : "residenceId already registered"
                    })
                } else {
                    sellerModel.addSeller(userObject, (results) => {
                        sellerModel.getById(results, (results) => {
                            res.status(201).json({
                                status : true,
